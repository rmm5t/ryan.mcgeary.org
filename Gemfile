source "https://rubygems.org"

gem "rake"
gem "middleman"
gem "middleman-blog"
gem "middleman-blog-drafts"      # adds `middleman draft` and `middleman publish`
gem "middleman-syntax"           # syntax highlighting with rouge
gem "middleman-alias", github: "rmm5t/middleman-alias", branch: "rel-canonical" # for redirects from old published URLs
gem "middleman-livereload"       # for development speed
gem "middleman-gh-pages"         # for publishing to GitHub pages
gem "middleman-google-analytics" # for all the measures
# gem "middleman-blog-similar"
# gem "levenshtein-ffi"            # for similar post algorithm
gem "builder"                    # For feed.xml.builder
gem "nokogiri"                   # For article summaries
